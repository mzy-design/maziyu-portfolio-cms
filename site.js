(async () => {
  ...
  if (page === 'project') document.head.insertAdjacentHTML('beforeend', `<link rel="stylesheet" href="${base}/project-bilingual.css">`);
  ...
  if (page === 'project') {
    ...
    root.innerHTML = `${nav}<main class="project-detail"><header class="project-detail-head"><p>${esc(p.number)} / ${esc(p.year)}</p><h1 class="${displayTitle.length > 14 ? 'is-long' : ''}">${esc(displayTitle)}${p.noteEn ? `<small>${esc(p.noteEn)}<span lang="zh-CN"> / ${esc(p.note)}</span></small>` : ''}</h1><div><span>${esc(p.subtitle)}</span><small>${p.services.map(item => esc(item)).join('<br>')}</small></div></header>${media(p.hero, p.title, 'project-hero')}<section class="project-overview" data-marquee="${esc(p.subtitle)}"><p>( Overview )</p><h2>${esc(p.overviewEn || p.overview || 'Project details to be confirmed')}${p.overviewEn ? `<small lang="zh-CN">${esc(p.overview)}</small>` : ''}</h2></section>${p.sections.map((section, index) => { const images = section.images || [section.image]; return `<section class="project-chapter${section.layout ? ` is-${esc(section.layout)}` : ''}"><header><span>${String(index + 1).padStart(2, '0')}</span><h2>${esc(section.headingEn || section.heading)}${section.headingEn ? `<small lang="zh-CN">${esc(section.heading)}</small>` : ''}</h2></header><p>${esc(section.bodyEn || section.body || 'Project details to be confirmed')}${section.bodyEn ? `<small lang="zh-CN">${esc(section.body)}</small>` : ''}</p><div class="project-chapter-media">${images.map((image, mediaIndex) => media(image, `${section.headingEn || section.heading} ${mediaIndex + 1}`)).join('')}</div></section>`; }).join('')}${p.noteBlock ? `<section class="project-note"><header><span>${esc(p.noteBlock.eyebrow)}</span><small>${esc(p.noteBlock.meta)}</small></header><p>${esc(p.noteBlock.textEn || p.noteBlock.text)}${p.noteBlock.textEn ? `<small lang="zh-CN">${esc(p.noteBlock.text)}</small>` : ''}</p></section>` : ''}<section class="project-gallery">${p.gallery.map((image, index) => media(image, `${p.title} gallery ${index + 1}`)).join('')}</section><section class="project-more"><header><h2>MORE WORKS</h2><a href="${base}/works/">SEE ALL (${detailed.length})</a></header><div class="project-more-grid">${moreWorks.map(item => `<a class="project-more-card" href="${base}/works/${esc(item.href)}"><img src="${esc(asset(item.image))}" alt="${esc(item.title)}" loading="lazy" decoding="async"><div><h3>${esc(item.title)}</h3><p>${esc(item.detailCategory || item.tags)}</p><small>${esc(item.noteEn)}</small><small lang="zh-CN">${esc(item.note)}</small></div></a>`).join('')}</div></section></main>`;
  }
