(async () => {
  ...
  if (page === 'works') {
    const projectCard = (item, index) => {
      ...
      return `<${tag} class="card${item.hasDetail ? '' : ' is-static'}"${href} data-category="${esc(item.category)}"><div><img src="${esc(asset(item.image))}" alt="${esc(item.title)}"><span>${String(index + 1).padStart(2, '0')}</span></div><h2>${esc(item.title)}</h2><p>${esc(item.tags)}</p><small>${esc(item.status)}</small></${tag}>`;
    };
