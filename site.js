(async () => {
  ...
  const [p, about] = await Promise.all([contentRequest, aboutRequest]);
  ...
  const aboutSectionsHtml = (page === 'about' || page === 'home') ? ((p) => `
    ...
    <section class="about-fan" aria-label="Drag to browse selected project images"><span class="fan-plus fan-plus-left" aria-hidden="true">＋</span><span class="fan-plus fan-plus-right" aria-hidden="true">＋</span><div class="fan-hint" aria-hidden="true"><svg viewBox="0 0 72 48"><path d="M8 29C13 11 43 5 59 18C67 25 66 35 59 42"/><path d="m52 39 8 3 1-8"/></svg><span>Drag to rotate</span></div><div class="fan-deck"><div class="fan-row" tabindex="0" aria-label="Drag horizontally or use arrow keys to browse projects">${fanItems.map(item => `<figure class="fan-card"><img src="${esc(asset(item.image))}" alt="${esc(item.title)}" draggable="false" loading="lazy" decoding="async"></figure>`).join('')}</div></div></section>
