(async () => {
  ...
  if (page === 'project' && !matchMedia('(prefers-reduced-motion:reduce)').matches) {
    const projectObserver = new IntersectionObserver(entries => entries.forEach(entry => {
      if (entry.isIntersecting) { entry.target.classList.add('is-visible'); projectObserver.unobserve(entry.target); }
    }), { threshold: .2 });
    document.querySelectorAll('.project-overview h2, .project-chapter header, .next-project strong').forEach(item => projectObserver.observe(item));
  }
