(async () => {
  ...
  const aboutSectionsHtml = (page === 'about' || page === 'home') ? ((p) => `
    ...
    <section class="about-fan" aria-label="Drag to browse selected project images"><span class="fan-plus fan-plus-left" aria-hidden="true">＋</span><span class="fan-plus fan-plus-right" aria-hidden="true">＋</span><div class="fan-deck"><div class="fan-row" tabindex="0" aria-label="Drag horizontally or use arrow keys to browse projects">${fanItems.map(item => `<figure class="fan-card"><img src="${esc(asset(item.image))}" alt="${esc(item.title)}" draggable="false" loading="lazy" decoding="async"></figure>`).join('')}</div></div></section>
